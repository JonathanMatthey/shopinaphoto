...
{url+="&";}
url+="chunkkey="+opts.chunkKey;url+="&data=";var chunkSize=$iTXT.core.Ajax.maxURLLength-url.length;var chunks=[];while(data.length>0){$iTXT.debug.trace($iTXT.debug.Category.CONTEXT,"Getting chunk "+chunks.length);var dataChunk;if(data.length<chunkSize)
...
$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,"Sending "+chunks.length+" Chunks To <b>"+url+"</b>");$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,"Using Chunk Key: <b>"+opts.chunkKey+"</b>");var chunksArr=[];for(var i=0;i<chunks.length;i++)
{var newUrl=url+chunks[i]+"&chunk="+i;newUrl+="&total="+chunks.length;var chunkDetails={loaded:false,number:i,url:newUrl,data:chunks[i]};$iTXT.debug.debug($iTXT.debug.Category.CONTEXT,"URL size before callback: "+newUrl.length);chunksArr.push(chunkDetails);}
